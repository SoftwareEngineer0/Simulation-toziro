...
public class Joystick : MonoBehaviour, IPointerUpHandler, IPointerDownHandler, IDragHandler 
{
	...
	void Start()
	{
		...
		ScreenSizeUpdate = new int[2];

		ScreenSize[0] = Screen.width;
		ScreenSize[1] = Screen.height;

		ScreenSizeUpdate[0] = Screen.width;
		ScreenSizeUpdate[1] = Screen.height;
	}

	void Update()
	{
		//スクリーンサイズを監視,変更があればstartPositionを更新
		ScreenSizeUpdate[0] = Screen.width;
		ScreenSizeUpdate[1] = Screen.height;

		if (ScreenSize[0] != ScreenSizeUpdate[0] || ScreenSize[1] != ScreenSizeUpdate[1]) 
		{
			ScreenSize[0] = Screen.width;
			ScreenSize[1] = Screen.height;
			startPosition = new Vector2(transform.position.x, transform.position.y);
